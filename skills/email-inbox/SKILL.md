...
**如果这台机器上还有别的 Agent 在用 cfmail，一律用 `--email <地址>` 指明邮箱，不要用 `cfmail use`**——「当前邮箱」是共享状态，几个 Agent 各自切换会互相覆盖。配置文件的并发写入本身是安全的（不会丢更新、不会读到半个文件）。
...
同一个归档目录同时只跑一个 sync，撞上就跳过这次（不算失败），所以定时任务可以放心设密一点。
