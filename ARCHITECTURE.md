...
- **配置文件按并发访问设计**：同一份配置会被同时驱动（定时的 `sync` 撞上手动的 `unread`）。写入走「临时文件 + `rename`」，因此读者永远看到完整的旧内容或完整的新内容，不会撞上被截断的一瞬间；「读—改—写」再包一把短锁（持有时间不到一毫秒），否则两个进程各读各写会丢掉先落地的那次更新。锁超过十秒视为崩溃遗留可被清除，且拿不到锁时仍继续写——写本身是原子的，最坏是丢一次更新而不是坏掉文件。
- **一份配置文件 = 一个邮箱**：曾经按地址存多个账号加一个 `current`（aws-cli / kubectl 的形状），但 `current` 是进程之间共享的可变状态——两个程序各自切换就会互相覆盖，而谁都不会收到通知。既然业务上没有「一条命令要跨多个邮箱」的场景，就把这个状态整个去掉：配置文件平铺存一个邮箱，第二个邮箱换一份文件，靠 `EMAIL_INBOX_CONFIG` 指定。密钥、未读游标、归档目录与通知 key 都在各自文件里，天然不会串。**旧的多账号文件按它选中的那个读**，下次写入自动转成扁平结构；存了不止一个的会列出来让用户拆分，不替他猜。管理端仍是单一配置：一个服务一把 `MCP_TOKEN`。
